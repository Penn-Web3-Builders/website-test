...
function TeamMember({ url, name, desc }) {
  return (
    <>
      <a href={url} target={url} className="m-3 flex flex-col items-center w-32 h-42 p-2 hover:shadow-lg rounded-lg box-border transition-shadow duration-500">
        <img className='w-24 h-28 object-cover rounded-sm' src="https://thumbs.dreamstime.com/b/golden-retriever-dog-21668976.jpg" ></img>
    ...
    <div className="flex flex-col items-center border-2 border-gray-300 mx-44 my-14 pb-2">
      ...
      <div className="flex justify-center flex-wrap">
        {teamMembers.map(member => {
          return (<TeamMember url={member.url} name={member.name} desc={member.desc} />)
        })}
      </div>
